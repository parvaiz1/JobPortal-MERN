...
function PostJobs() {
    ...
    async function postJob() {
       ...
       let jobLocation = joblocation.toLowerCase()
        await axios.post("http://ec2-15-206-28-178.ap-south-1.compute.amazonaws.com:8080/jobpost/jobpost/", { Logo, SourceLink, Source, empId, jobTitle, companyName, jobDescription, jobtype, salaryRange, jobLocation, qualification, experiance, skills })
            .then((res) => {
                let result = (res.data)
                if (result == "success") {
                    setJobTitle("")
